...
from app.models import OrganizationMembership, MembershipRole
...
class testdata:
    ...
    def addyaml(yamlfile="install/testdata.yaml"):  # noqa
        ...
        for org in data_loaded['org']:
            ...
            if ('parent_org' in org):
                po = Organization.query.filter_by(
                    abbreviation=org['parent_org']).first()
                o.parent_org = po
            db.session.add(o)
            db.session.commit()
    ...
    def print():
        ...
        for oxu in u.get_organization_memberships():
            ...
            print('%s %s %s' % (oxu.email, oxu.membership_role.name,
