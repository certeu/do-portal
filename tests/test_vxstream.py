...
def test_get_html_report(client, monkeypatch):
    ...
    rv = client.get(
        url_for(
            'api.get_vxstream_report', sha256='sss', envid=1, type_='html'))
...
def test_get_vxstream_json_summary(client, monkeypatch, malware_sample):
    ...
    rv = client.get(url_for('api.get_vxstream_analysis', sha256=1, envid=1))
    assert rv.status_code == 200
...
def test_download_cp_html_report(client, monkeypatch):
    ...
    rv = client.get(
        url_for(
            'api.get_vxstream_download', sha256='sss', eid=1, ftype='html'))
    assert rv.status_code == 200
