...
def test_get_cp_vxstream_json_summary(client, monkeypatch, malware_sample):
    vxstream = MagicMock()
    state_resp = MagicMock(return_value={
        'response': {
            'state': _state_to_name[SUCCESS]
        },
        'response_code': 0
    })
    ...
    vxstream.side_effect = [state_resp(), summary_resp()]
    monkeypatch.setattr(BaseQuery, 'first_or_404', lambda x: True)
    monkeypatch.setattr(VxAPIClient, 'get', lambda *args, **kw: vxstream())
    rv = client.get(url_for('cp.get_cp_vxstream_analysis', sha256=1, envid=1))
...
def test_download_cp_html_report(client, monkeypatch):
    ...
    rv = client.get(
        url_for(
            'cp.get_cp_vxstream_download', sha256='sss', eid=1, ftype='html'))
    assert rv.status_code == 200
...
def test_get_cp_html_report(client, monkeypatch):
    ...
    rv = client.get(
        url_for(
            'cp.get_cp_vxstream_report', sha256='sss', envid=1, type_='html'))
    assert rv.status_code == 200
...
def test_create_fqdn(client):
    rv = client.post(url_for('cp.add_fqdn'), json=dict(fqdn='cert.europa.eu'))
